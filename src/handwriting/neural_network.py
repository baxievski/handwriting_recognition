...
class NeuralNetwork:
    def __init__(self, input_nodes, hidden_nodes, output_nodes, learning_rate):
        param_size = (input_nodes + 1) * hidden_nodes + (
            hidden_nodes + 1
        ) * output_nodes
    ...
    def backward_propagate(self, X, y):
        ...
        J += (
            ...
            / (2 * m)
            * (
                np.sum(np.power(self.theta1[:, 1:], 2))
                + np.sum(np.power(self.theta2[:, 1:], 2))
            )
        )
        ...
        for t in range(m):
            ...
            d2t = np.multiply((self.theta2.T * d3t.T).T, self.sigmoid_gradient(z2t))
    ...
    def train(
        ...
        for i in range(iterations):
            ...
            self.theta1 -= self.learning_rate * np.matrix(
                np.reshape(
            ...
            self.theta2 -= self.learning_rate * np.matrix(
                np.reshape(
                    grad[self.hidden_nodes * (self.input_nodes + 1) :],
            ...
            if i % batch_size == 0:
                train_acc_history.append(
                    (self.predict_values(X_batch) == y_batch_values).mean()
                )
                ...
                if X_test is not None and y_test is not None:
                    ...
                    test_acc_history.append(
                        (predicted_test_values == y_test_values).mean()
                    )
                ...
                if verbose:
                    print(
                        "i: {: 4d} J: {: 6.4f} train acc.: {: 6.4f} test acc.: {: 6.4f} learn rate: {: 6.5f}".format(
                            i,
                            J,
                            train_acc_history[-1],
                            test_acc_history[-1],
